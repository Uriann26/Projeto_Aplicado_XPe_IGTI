...
DROP POLICY IF EXISTS "Users can view their own notification schedules" ON notification_schedules;
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_notification_schedules_updated_at'
  ) THEN
...
CREATE INDEX IF NOT EXISTS notification_schedules_notification_id_idx ON notification_schedules(notification_id);
CREATE INDEX IF NOT EXISTS notification_schedules_scheduled_for_idx ON notification_schedules(scheduled_for);
...
DROP TRIGGER IF EXISTS schedule_deadline_reminder_trigger ON deadlines;
